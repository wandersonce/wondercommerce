...
export default function StoreSwitcher({ className, items = []} : StoreSwticherProps) {
            ...
            <CommandGroup heading="Stores">
              {formattedItems.map((store) => (
                ...
                >
                  <StoreIcon className="mr-2 h-4 w-4" />
                  {store.label}
                  <CheckIcon className={cn("ml-auto w-4 h-4", currentStore?.value === store.value ? "opacity-100" : "opacity-0")}/>
                </CommandItem>
              ))}
            </CommandGroup>
          </CommandList>
